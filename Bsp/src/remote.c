...
/* 	
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_0;				 //PE0 输入 
 	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IN_FLOATING; 		//输入 
 	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
 	GPIO_Init(GPIOE, &GPIO_InitStructure);
 	GPIO_SetBits(GPIOE,GPIO_Pin_0);	//初始化GPIOE.0
*/					  
		...
		{			
			...
			if(RmtSta&0X10)					//完成一次高电平捕获 
			{
 				if(RmtSta&0X80)//接收到了引导码
				{
					...
					{
//						RmTCnt++; 		//按键次数增加1次
						if(RmTCnt<0xff)RmTCnt++; 		//按键次数增加1次
						RmtSta&=0XF0;	//清空计时器		
					}
 				}
...
L         :长按键
H         :一直在按键
kkkk      :键值,为0000,没有任何按键按下
0000 xxxx :0LHS=0000键放开
***************************************************/
...
{        
	...
	{ 
	    ...
	    t3=(RmtRec>>8)&0xff;				//得到Key反码 
	    t4= RmtRec &0xff;					//得到Key
// 	    if((t1==(u8)~t2)&&t1==REMOTE_ID)	//检验遥控识别码(ID)及地址 
 	    if((t1==(u8)~t2) && (t3 == (u8)~t4))//检验遥控码
	    { 
	        t1=RmtRec>>8;
	        t2=RmtRec; 	
	        if(t1==(u8)~t2)
