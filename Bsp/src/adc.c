...
void T_Adc_Init(void)  //ADC通道初始化
{
	...
	RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOA |RCC_APB2Periph_ADC1|RCC_APB2Periph_ADC2|RCC_APB2Periph_ADC3, ENABLE );	  //使能ADC1通道时钟
...
void Get_Adc_Average(void)
{
    ...
    while(cnt--)
    {
        ADC_SoftwareStartConvCmd(ADC1, ENABLE);							//使能指定的ADC1的软件转换启动功能
        while(!ADC_GetFlagStatus(ADC1, ADC_FLAG_EOC ));					//等待转换结束
        AdcTemp[cnt] = ADC_GetConversionValue(ADC1);								//返回最近一次ADC1规则组的转换结果
    }
    AvVal[1] = GetMedianNum(AdcTemp, CNT);
    ...
    while(cnt--)
    {
        ...
        AdcTemp[cnt] = ADC_GetConversionValue(ADC2);								//返回最近一次ADC1规则组的转换结果
    }
    AvVal[2] = GetMedianNum(AdcTemp, CNT);
    ...
    for(i = 0; i < 5; i++)
    {
        ...
        while(cnt--)
        {
            ADC_RegularChannelConfig(ADC3, chlist[i], 1, ADC_SampleTime_239Cycles5 );	//ADC1,ADC通道3,规则采样顺序值为1,采样时间为239.5周期
            ADC_SoftwareStartConvCmd(ADC3, ENABLE);							//使能指定的ADC1的软件转换启动功能
            while(!ADC_GetFlagStatus(ADC3, ADC_FLAG_EOC ));					//等待转换结束
            AdcTemp[cnt] = ADC_GetConversionValue(ADC3);								//返回最近一次ADC1规则组的转换结果
        }
        AvVal[chlist[i]] = GetMedianNum(AdcTemp, CNT);
...
/*void Get_Adc_Average(void)
{
	...
	{
        while(!ADC_GetFlagStatus(ADC2, ADC_FLAG_EOC ));					//等待转换结束
		adcx = ADC_GetConversionValue(ADC2);								//返回最近一次ADC1规则组的转换结果
		temp_val[adch] += adcx;
	}
    else if(adch==_SD)//给定电流
	{
        while(!ADC_GetFlagStatus(ADC3, ADC_FLAG_EOC ));					//等待转换结束
		adcx = ADC_GetConversionValue(ADC3);								//返回最近一次ADC1规则组的转换结果
		temp_val[adch] += adcx;
	}
	else
	{
        ...
        if(temp_val1[adch] > temp_val[adch])
        {
        ...
        {
            if((temp_val[adch] - temp_val1[adch])<5)			//限幅滤波法
            {
                temp_val[adch]=temp_val1[adch];
            }
        }
